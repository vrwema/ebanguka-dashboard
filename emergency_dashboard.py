...
@st.cache_data
def load_data():
    """Load and preprocess the emergency transfer data"""
    # Try to load the latest CSV file, fallback to existing file if not available
    try:
        df = pd.read_csv('ebanguka_data_latest.csv')
    except FileNotFoundError:
        df = pd.read_csv('ebanguka_data_20250908_120931.csv')
